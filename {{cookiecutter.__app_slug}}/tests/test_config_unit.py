...
def test_config_dictionary_not_in_schema(place_test_config, tmp_path, caplog: pytest.LogCaptureFixture):
    ...
    assert "Found config entry key <root>[TEST_CONFIG_ROOT_ENTRY_NOT_IN_SCHEMA] that is not in schema" in caplog.text
    assert "Found config entry key [app][TEST_CONFIG_APP_ENTRY_NOT_IN_SCHEMA] that is not in schema" in caplog.text
